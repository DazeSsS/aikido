  ...
  useEffect(() => {
    const fetchPractices = async () => {
      ...
      const res = await getApiResource(
        API_URL +
          `${view}/practices?offset=${currentOffset}&past=${
  ...
  return (
    <>
      <ControlsPanel
        ...
        leftArrowState={
          view === 'student' && scheduleView === 'schedule'
            ? currentOffset
            : null
        }
        rightArrowState={
          view === 'student' && scheduleView === 'student-attendance'
            ? currentOffset
            : null
        }
      />
        ...
        <div className={styles['schedule-items__container-grid']}>
          <ul className={styles['schedule-items__container']}>
            {isLoading ? (
              ...
              <h4 className={styles['empty-alert']}>
                Тренировок на эту неделю нет
              </h4>
            ) : (
              practices &&
                    ...
                    {dayPractices.length === 0 ? (
                      ...
                      dayPractices.map((practice) => (
                        <div
                          ...
                          className={
                            styles['schedule-item-practice__container']
                          }
                        >
                          ...
                          {scheduleView === 'student-attendance' && (
                            <img
                              ...
                              src={
                                practice.attended.includes(Number(getUserId()))
                                  ? '/tick.svg'
                                  : '/cross.svg'
                              }
                              alt=""
                          ...
                          <div className={styles['practice-info']}>
                            <div className={styles['practice-info-item']}>
                              <h4>Место: </h4>
                              <h4
                                className={styles['practice-info-item-value']}
                              >
                            ...
                            <div className={styles['practice-info-item']}>
                              ...
                              <h4
                                className={styles['practice-info-item-value']}
                              >
                                {dayjs(practice.date).format('HH:mm')} -{' '}
                                {dayjs(practice.end_time).format('HH:mm')}
                              </h4>
                            </div>
                          </div>
                        </div>
